...
//             return res.status(200).json({success:true,msg:`OTP Verified! Welcome ${newuser.user_name}`,token});
...
const resendotp = async (req,res) => {
    try {
        ...
        if(!result)
            return res.status(500).json({sucess:false,msg:"error in sending mail"});
